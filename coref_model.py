...
class CorefModel(object):
  ...
  def get_predictions_and_loss(self, tokens, context_word_emb, head_word_emb, lm_emb, char_index, text_len, speaker_ids, genre, is_training, gold_starts, gold_ends, gold_cluster_ids, gold_types):
    ...
    with tf.variable_scope("mention_attention"):
      ...
      top_span_att_scores = tf.squeeze(top_span_att_scores, axis=1)


    cluster_scores, cluster_indices, individual_cluster_size, predicted_antecedents,predicted_mention_types = \
      self.get_cluster_scores(top_span_emb, top_span_mention_scores, top_span_att_scores, top_span_speaker_ids, genre_emb,mention_type_scores)
  ...
  def get_cluster_scores(self, mention_emb, mention_scores, mention_att_scores, mention_speaker_ids, genre_emb,mention_type_scores):
    ...
    max_mention_per_cluster = 50
    init_mention_sid = tf.range(0, num_mentions, dtype=tf.int32)
    init_cluster_size = tf.zeros([max_clusters], dtype=tf.int32)
    init_cluster_last_mention = tf.zeros([max_clusters], dtype=tf.int32)
    init_cluster_type = tf.zeros([max_clusters],dtype=tf.int32)
    init_cluster_len = tf.constant(0, dtype=tf.int32)
    init_cluster_scores = tf.zeros([max_clusters + self.n_types])
    init_cluster_indices = tf.zeros([max_clusters, max_mention_per_cluster], dtype=tf.int32)
    init_cluster_emb = tf.zeros([max_clusters, mention_emb_size])
    init_cluster_att_scores = tf.zeros([max_clusters])
    init_cluster_mention_scores = tf.zeros([max_clusters])
    init_cluster_sid = tf.zeros([max_clusters], dtype=tf.int32)

    def _cluster_scan(pre, input):
      m_emb, m_init_sid, m_score, m_att_score, m_type_score = input
      dim_m_emb, dim_m_init_sid, dim_m_score, dim_m_att_score = \
        tf.expand_dims(m_emb, 0), tf.expand_dims(m_init_sid, 0) \
          , tf.expand_dims(m_score, 0),tf.expand_dims(m_att_score, 0)
      cl_emb, cl_att_score, cl_m_score, cl_indices, cl_size, cl_sid, cl_last_m, cl_type, cl_len, _, _, _, _, _ = pre

      feature_emb_list = []
      with tf.variable_scope("feature_emb", reuse=tf.AUTO_REUSE):
        if self.config['use_metadata']:
          cl_speaker_ids = tf.gather(mention_speaker_ids, cl_last_m)  # [max_ant]
          ...
          tiled_genre_emb = tf.tile(tf.expand_dims(genre_emb, 0),
                                    [max_clusters, 1])  # [max_ant, emb]
          feature_emb_list.append(tiled_genre_emb)

        if self.config['use_features']:
          mention_distance = dim_m_init_sid - cl_last_m  # [max_ant]
        ...
        cluster_width_bins = coref_ops.cluster_width_bins(cl_size)
      ...
      target_emb_tiled = tf.tile(dim_m_emb,
                                 [max_clusters, 1])  # [max_ant, emb]
      similarity_emb = cl_emb * target_emb_tiled  # [max_ant, emb]

      pair_emb = tf.concat([target_emb_tiled, cl_emb, similarity_emb, feature_emb],
                           1)  # [max_ant, emb]
      ...
      cluster_scores += tf.log(
        tf.sequence_mask(cl_len, max_clusters, dtype=tf.float32))  # [max_ant]
      cluster_scores += dim_m_score + cl_m_score  # [max_ant]
      ...
      def _exclude():
        return cl_emb,cl_att_score, cl_m_score,cl_indices,cl_size,cl_sid,cl_last_m,cl_type, cl_len,-1,step

      def _dn():
        indices = tf.range(0, max_clusters, dtype=tf.int32)
        re_cl_emb = tf.gather(tf.concat([dim_m_emb, cl_emb], axis=0), indices)
        re_cl_att_score = tf.gather(tf.concat([dim_m_att_score, cl_att_score], axis=0), indices)
        ...
        re_cl_sid = tf.gather(tf.concat([dim_m_init_sid, cl_sid], axis=0), indices) if self.config['use_history_clusters'] else cl_sid
        re_cl_last_m = tf.gather(tf.concat([dim_m_init_sid, cl_last_m], axis=0), indices)
        re_cl_type = tf.gather(tf.concat([tf.expand_dims(step,axis=0), cl_type],axis=0),indices)
        re_cl_len = tf.minimum(max_clusters, cl_len + 1)
        re_ant = -1
        return re_cl_emb, re_cl_att_score, re_cl_m_score, re_cl_indices, re_cl_size, re_cl_sid, re_cl_last_m, re_cl_type, re_cl_len, re_ant,step
      ...
      def _do():
        if self.config['use_history_clusters']:
          ...
          indices = tf.concat([tf.range(0, act_step + 1), tf.range(act_step + 2, max_clusters + 1)], axis=0)

        expanded_step = tf.expand_dims(act_step, axis=0)
        att = tf.nn.softmax(tf.concat([tf.gather(cl_att_score, expanded_step), dim_m_att_score], axis=0), axis=0)
        step_cl_emb = tf.reduce_sum(
          tf.concat([tf.gather(cl_emb, expanded_step), dim_m_emb], axis=0) * tf.expand_dims(att, axis=1),
          axis=0)
        step_cl_m_score = tf.reduce_sum(
          tf.concat([tf.gather(cl_m_score, expanded_step), dim_m_score], axis=0) * att,
          axis=0)
        ...
        step_att_score = tf.gather(cl_att_score, expanded_step) + dim_m_att_score
        re_cl_att_score = tf.gather(tf.concat([step_att_score, cl_att_score], axis=0), indices)

        step_cl_indices = tf.gather(tf.concat([dim_m_init_sid, tf.gather(cl_indices, act_step)], axis=0),
                                    tf.range(0, max_mention_per_cluster, dtype=tf.int32))  # [max_clusters]
        re_cl_indices = tf.gather(tf.concat([tf.expand_dims(step_cl_indices, axis=0), cl_indices], axis=0), indices)
        re_cl_size = tf.gather(tf.concat([tf.minimum(max_mention_per_cluster, tf.gather(cl_size, expanded_step) + 1), cl_size], axis=0), indices)
        ...
        re_cl_last_m = tf.gather(tf.concat([dim_m_init_sid, cl_last_m], axis=0), indices)
        re_cl_type = tf.gather(tf.concat([[self.n_types],cl_type],axis=0),indices)
        re_cl_len = tf.minimum(max_clusters, cl_len + 1) if self.config['use_history_clusters'] else cl_len
        re_ant = tf.gather(cl_last_m, act_step)
        return re_cl_emb, re_cl_att_score, re_cl_m_score, re_cl_indices, re_cl_size, re_cl_sid, re_cl_last_m, re_cl_type, re_cl_len, re_ant,-1

      prefiltering_threshold = self.config['prefiltering_threshold']
      re_cl_emb,re_cl_att_score,re_cl_m_score, re_cl_indices, re_cl_size, re_cl_sid, re_cl_last_m, re_cl_type, re_cl_len, re_ant, re_m_type = \
      ...
      re_cl_att_score.set_shape([None])
      ...
      re_cl_type.set_shape([None])
      re_cl_len.set_shape([])
      re_ant.set_shape([])
      re_m_type.set_shape([])
      cluster_scores.set_shape([None])
      return re_cl_emb, re_cl_att_score, re_cl_m_score, re_cl_indices, re_cl_size, re_cl_sid, re_cl_last_m, re_cl_type, re_cl_len, re_ant, re_m_type, cluster_scores, cl_indices, cl_size

    _,_, _, _,_, _, _, _,  _, predicted_antecedents, predicted_mention_types, cluster_scores, cluster_indices, individual_cluster_size = tf.scan(
        _cluster_scan,
      (mention_emb, init_mention_sid, mention_scores,mention_att_scores, mention_type_scores),
      initializer=(
        init_cluster_emb, init_cluster_att_scores,init_cluster_mention_scores,
        init_cluster_indices, init_cluster_size,
        init_cluster_sid, init_cluster_last_mention, init_cluster_type,

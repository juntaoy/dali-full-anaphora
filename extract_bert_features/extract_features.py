...
flags.DEFINE_string("corpus","CONLL","")
...
def main(_):
    ...
    json_examples = []
    if FLAGS.corpus == 'RST' or FLAGS.corpus == 'CONLL' or FLAGS.corpus == 'PD':
        dset = ['test', 'train', 'dev']
    else:
        dset = ['']
    for x in dset:
        if FLAGS.corpus == "CONLL":
            cfile = os.path.join(FLAGS.input_file, x + '.english.jsonlines')
        elif FLAGS.corpus == "RST":
            cfile = os.path.join(FLAGS.input_file, x + '.english.rst.jsonlines')
        elif FLAGS.corpus == "PD":
            cfile = os.path.join(FLAGS.input_file, x+'.english.pd2.0.crac.jsonlines')
        else:
            cfile = FLAGS.input_file
